...
class Compiler:
    ...
    def compile_expr_to(self, dst: str, expr: str):
        ...
        if m:
            ...
            if op == "+":
                ...
                self.emit(f"mul {dst} = {self.value(left)},{self.value(right)}")
            elif op == "/":
                self.emit(f"div {dst} = {self.value(left)},{self.value(right)}")
            else:
    ...
    def compile(self, src: str) -> str:
        self.out.append("// generated by ia64flow.compiler")
        self.out.append("")
...
def compile_flow(src: str) -> str:
    compiler = Compiler()
    return compiler.compile(src)


def compile_file(in_path: str | Path, out_path: str | Path | None = None) -> Path:
