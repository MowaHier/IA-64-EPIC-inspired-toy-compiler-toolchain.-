...
def reads_writes(ins: Insn) -> tuple[set[str], set[str], set[str], set[str]]:
...
class Checker:
    def __init__(self, path: Path, src: str):
        ...
        self.groups: list[list[Insn]] = []

    def error(self, line_no: int, text: str):
        self.messages.append(Message(line_no, "error", text))

    def warning(self, line_no: int, text: str):
        self.messages.append(Message(line_no, "warning", text))
    ...
    def check_insn_shape(self, ins: Insn):
    ...
    def check_group(self, group: list[Insn]):
        if len(group) > 3:
            self.error(
                group[3].line_no,
        ...
        reg_writers: dict[str, Insn] = {}
        pred_writers: dict[str, Insn] = {}
        ...
        for ins in group:
            ...
            for r in sorted(reg_reads & previous_reg_writes):
                self.error(
                    ins.line_no,
    ...
    def check(self):
        ...
        # Only do dependency checks after shape checks.
        # Otherwise malformed instructions may crash reads_writes().
    ...
    def print_messages(self):
        for m in self.messages:
            print(f"{self.path}:{m.line_no}: {m.level}: {m.text}")
